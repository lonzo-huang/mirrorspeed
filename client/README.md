...
| 多设备指纹（v2.3.3） | 按硬件指纹区分同账号多终端；免费 2 台 / 会员 5 台，超限按系统语言提示并引导升级 |
| 配额提示（v2.3.2） | 免费用户当日流量超额（被挂起）时首页横幅提示：明日刷新 / 升级 / 看广告 |
| Windows 桌面体验（v2.3.4–2.3.6） | 连上自动设置隧道 DNS（修"通但打不开网页"）；单实例；系统托盘常驻（点 X 最小化、菜单显示/退出）；固定手机尺寸窗口（禁最大化/拉伸） |
| 在线更新提示（v2.3.6） | 启动检测新版本软提示弹窗；支持 `min_supported_version` 强制更新（运营在 Supabase 配置，见根 README §6.2） |

> **运营开关**（公告、强制更新最低版本、免费额度、分级限速、超级用户等）集中在 Supabase
> `app_config` 表，无需发版即可调整 —— 完整键位与 SQL 示例见 **根目录 `README.md` §6「运营配置」**。
