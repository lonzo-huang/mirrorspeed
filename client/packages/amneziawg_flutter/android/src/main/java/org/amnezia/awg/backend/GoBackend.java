...
public final class GoBackend implements Backend {
    ...
    private void setStateInternal(final Tunnel tunnel, @Nullable final Config config, final State state)
        ...
        if (state == State.UP) {
            ...
            // protect() binds the socket to the physical network interface (SO_BINDTODEVICE),
            // which prevents the socket from reaching loopback addresses (127.x.x.x).
            // When the peer endpoint is loopback (relay mode via WsRelayService), skip
            // protect() so the kernel's local routing table (priority 0) handles the
            // loopback traffic correctly. For all other endpoints, protect() is still
            // required to prevent the WireGuard socket from being caught by VPN rules.
            boolean hasLoopbackEndpoint = false;
            for (final Peer peer : config.getPeers()) {
                final InetEndpoint ep = peer.getEndpoint().orElse(null);
                if (ep != null && ep.getHost().startsWith("127.")) {
                    hasLoopbackEndpoint = true;
                    break;
                }
            }
            if (!hasLoopbackEndpoint) {
                service.protect(awgGetSocketV4(currentTunnelHandle));
                service.protect(awgGetSocketV6(currentTunnelHandle));
            } else {
                Log.d(TAG, "Loopback endpoint detected — skipping socket protect() for relay mode");
            }
