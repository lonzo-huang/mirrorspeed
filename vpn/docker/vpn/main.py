"""
MirrorSpeed VPN Management API
Runs inside the WireGuard container, shares its network namespace.
...
def parse_peers_from_conf() -> list[dict]:
    """
    Parse [Peer] sections from wg0.conf.
    Each peer block may start with a comment: # Name = <peer_name>
    ...
    for block in raw_blocks:
        ...
        for line in block.splitlines():
            line = line.strip()
            # Check for name comment: # Name = foo
...
def get_live_stats() -> dict[str, dict]:
    """
    Run `wg show wg0 dump` and return dict keyed by public_key with runtime stats.
...
def syncconf():
    """Hot-reload WireGuard config without dropping existing peers."""
    result = subprocess.run(
        ["bash", "-c", f"wg syncconf {WG_IFACE} <(wg-quick strip {WG_IFACE})"],
        capture_output=True, text=True, check=False
    )
    if result.returncode != 0:
        raise HTTPException(
            status_code=500,
            detail=f"wg syncconf failed: {result.stderr.strip()}"
        )


def append_peer_to_conf(name: str, public_key: str, preshared_key: str, vpn_ip: str):
    """Append a new [Peer] block to wg0.conf."""
    block = (
        f"\n# Name = {name}\n"
        f"[Peer]\n"
...
def health():
    ...
    wg = subprocess.run(["wg", "show", WG_IFACE], capture_output=True, check=False)
    return {
...
def list_peers(_key: str = Security(verify_api_key)):
    """List all configured peers with live WireGuard stats."""
    with _conf_lock:
        peers = parse_peers_from_conf()
...
def create_peer(req: CreatePeerRequest, _key: str = Security(verify_api_key)):
    """Create a new WireGuard peer and return its keys + VPN IP."""
...
def delete_peer(peer_name: str, _key: str = Security(verify_api_key)):
    """Remove a peer from WireGuard config and hot-reload."""
...
def set_peer_status(peer_name: str, req: SetActiveRequest, _key: str = Security(verify_api_key)):
    ...
    with _conf_lock:
        ...
        if req.active:
            # Restore original VPN IP — find it from the wg0.conf comment block if blocked
