...
export function SiteNav() {
  ...
  const navLink = (href: string, label: string) => (
    <Link
      href={href}
      className={`hover:text-foreground transition-colors ${pathname === href ? "text-foreground" : "text-muted-foreground"}`}
    >
      {label}
    </Link>
  );
  ...
  return (
    <nav className="sticky top-0 z-50 glass-panel border-b border-border py-3 px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/login"  className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">{t.nav.login}</Link>
        <Link href="/login"  className="text-sm font-semibold bg-primary text-primary-foreground px-4 py-2 rounded-full hover:bg-mirror transition-all">{t.nav.signup}</Link>
      </div>
    </nav>
  );
}
